...
export function useExportData() {
  ...
  const exportToPDF = (entries: Entry[], dateRange: DateRange, dailyStats: DailyStats, rateCash: number, rateCard: number, masterName: string = 'Мастер') => {
    ...
    const tableRowsHtml = sortedDates.map(dateKey => {
      ...
      const summaryRow = `
        <tr class="daily-summary-row" style="page-break-inside: avoid;">
          <td colspan="4" style="background-color: #f1f2f6; text-align: right; font-weight: bold; color: #7f8c8d; font-size: 11px; padding: 8px; border-top: 1px solid #bdc3c7; border-bottom: 1px solid #bdc3c7;">ИТОГО ЗА ДЕНЬ:</td>
          <td colspan="3" style="background-color: white; font-weight: bold; color: ${color}; font-size: 13px; padding: 8px; border-left: 1px solid #bdc3c7; border-top: 1px solid #bdc3c7; border-bottom: 1px solid #bdc3c7;">
      ...
      const totalsRow = `
        <tr class="daily-totals-row" style="page-break-inside: avoid;">
          <td colspan="7" style="background-color: #f1f2f6; text-align: center; font-weight: bold; color: #7f8c8d; font-size: 11px; padding: 10px; border: 1px solid #bdc3c7;">
            <span style="margin-right: 40px;">ОБЩЕЕ: <span style="color: #34495e; font-size: 12px;">${dayTotal.toFixed(2)}€</span></span>
            <span style="margin-right: 40px;">КАРТОЙ: <span style="color: #34495e; font-size: 12px;">${dayTotalCard.toFixed(2)}€</span></span>
            <span>НАЛИЧНЫМИ: <span style="color: #34495e; font-size: 12px;">${dayTotalCash.toFixed(2)}€</span></span>
          </td>
        </tr>
      `;
